					    ...
					    int num)
{
	...
	for (cpu = 0; cpu < port->nthreads; cpu++) {
	...
	if (desc_count >
	   (txq->size - (port->nthreads * MVPP2_CPU_DESC_CHUNK)))
		return -ENOMEM;
			    ...
			    struct mvpp2 *priv)
{
	...
	port->nthreads = has_tx_irqs ? min_t(unsigned int, num_present_cpus(),
					     MVPP2_MAX_THREADS) : 1;
