...
#define MVPP2_DMA_MASK	DMA_BIT_MASK(40)
					    ...
					    struct mvpp2_tx_desc *tx_desc)
{
	if (port->priv->hw_version == MVPP21)
		...
		return tx_desc->pp22.buf_dma_addr_ptp & MVPP2_DMA_MASK;
}
				      ...
				      dma_addr_t dma_addr)
{
	...
	if (port->priv->hw_version == MVPP21) {
		...
		tx_desc->pp22.buf_dma_addr_ptp &= ~MVPP2_DMA_MASK;
					    ...
					    struct mvpp2_rx_desc *rx_desc)
{
	if (port->priv->hw_version == MVPP21)
		...
		return rx_desc->pp22.buf_dma_addr_key_hash & MVPP2_DMA_MASK;
}
					     ...
					     struct mvpp2_rx_desc *rx_desc)
{
	if (port->priv->hw_version == MVPP21)
		...
		return rx_desc->pp22.buf_cookie_misc & MVPP2_DMA_MASK;
}
...
static int mvpp2_probe(struct platform_device *pdev)
{
	...
	if (priv->hw_version == MVPP22) {
		err = dma_set_mask(&pdev->dev, MVPP2_DMA_MASK);
		if (err)
			goto err_mg_clk;
