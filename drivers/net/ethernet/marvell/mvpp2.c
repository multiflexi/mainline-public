...
/* There is TCAM range reserved for MAC entries, range size is 80
				  ...
				  enum mvpp2_prs_l2_cast type)
{
	...
	for (tid = MVPP2_PE_MAC_RANGE_START;
		...
		if (type == MVPP2_PRS_L2_UNI_CAST && !is_unicast_ether_addr(da))
			continue;

		if (type == MVPP2_PRS_L2_MULTI_CAST &&
...
static void mvpp2_set_rx_mc_promisc(struct mvpp2_port *port, bool enable)
{
	if (!enable)
		mvpp2_prs_mac_del_all(port, MVPP2_PRS_L2_MULTI_CAST);

	mvpp2_prs_mac_promisc_set(port->priv, port->id,
				  MVPP2_PRS_L2_MULTI_CAST, enable);
}

static void mvpp2_set_rx_uc_promisc(struct mvpp2_port *port, bool enable)
{
	if (!enable)
		mvpp2_prs_mac_del_all(port, MVPP2_PRS_L2_UNI_CAST);
...
static void mvpp2_set_rx_promisc(struct mvpp2_port *port, bool enable)
{
	...
	mvpp2_set_rx_mc_promisc(port, enable);
	mvpp2_set_rx_uc_promisc(port, enable);
}
...
static void mvpp2_set_rx_mode(struct net_device *dev)
{
	...
	if (netdev_uc_count(dev) > MVPP2_PRS_MAC_UC_FILT_MAX ||
		...
		mvpp2_set_rx_uc_promisc(port, true);

	if (dev->flags & IFF_ALLMULTI) {
		mvpp2_set_rx_mc_promisc(port, true);
		return;
	}
