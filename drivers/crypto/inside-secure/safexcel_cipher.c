				  ...
				  u32 length)
{
	...
	if (ctx->mode == CONTEXT_CONTROL_CRYPTO_MODE_CBC) {
		...
		case SAFEXCEL_DES:
			...
			memcpy(cdesc->control_data.token, req->iv, DES_BLOCK_SIZE);
			cdesc->control_data.options |= EIP197_OPTION_2_TOKEN_IV_CMD;
			break;

		case SAFEXCEL_3DES:
			offset = DES3_EDE_BLOCK_SIZE / sizeof(u32);
			memcpy(cdesc->control_data.token, req->iv, DES3_EDE_BLOCK_SIZE);
			cdesc->control_data.options |= EIP197_OPTION_2_TOKEN_IV_CMD;
			break;
...
static int safexcel_queue_req(struct skcipher_request *req,
